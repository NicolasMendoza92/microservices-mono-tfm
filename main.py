...
#     """
#     Recibe un archivo CV (PDF, DOCX, TXT), lo procesa con PLN,
#     evalúa la empleabilidad y recomienda puestos.
#     """
#     candidate_id = str(uuid4()) # Generar un ID único para el candidato
...
#         # 2. Procesamiento de CV (PLN con Hugging Face)
#         # La llamada permanece igual gracias al alias en la importación
#         cv_data_processed = await process_cv_with_pln(file_location) 
#         processed_candidates_db[candidate_id] = CVProcessedData(**cv_data_processed)

#         # 3. Evaluación de empleabilidad
#         employability_results = await predict_employability(cv_data_processed)

#         # 4. Recomendación de puestos
#         job_recommendations = await recommend_jobs(cv_data_processed)

#         # 5. Microservicio opcional: Preparación de entrevista
#         interview_questions = await generate_interview_questions(
#             candidate_name=cv_data_processed.get("name", "Candidato"),
#             skills=cv_data_processed.get("skills", []),
#             experience=cv_data_processed.get("experience", []),
#             areas_for_development=employability_results.get("areas_for_development", []),
#             job_recommendations=job_recommendations
#         )

#         # Construir el resumen del candidato
#         summary = CandidateSummary(
#             id=candidate_id,
#             name=cv_data_processed.get("name", "Candidato Desconocido"),
#             employability_score=employability_results["employability_score"],
#             top_recommendations=job_recommendations,
#             last_processed=datetime.datetime.now().isoformat(),
#             areas_for_development=employability_results["areas_for_development"],
...
#         raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
#     except Exception as e:
#         print(f"Error inesperado al procesar CV: {e}")
#         raise HTTPException(
#             status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
#             detail=f"Error interno del servidor al procesar el CV: {e}"
#         )
#     finally:
#         # Opcional: Eliminar el archivo después de procesar si no necesitas persistirlo
#         if os.path.exists(file_location):
#             os.remove(file_location)
